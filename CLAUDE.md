...
### Сервер PriceForge
- Проект развёрнут на **собственном выделенном VPS**
- **НЕ использует** общий n8n-сервер, общий PostgreSQL, общий Redis других проектов
- Полностью изолированная инфраструктура: свой VPS, своя БД, свой Redis
...
### Правила изоляции
1. **НЕ подключаться** к серверам других проектов из этого портфолио
2. **НЕ использовать** API-ключи, креды или конфиги из других проектов
3. **НЕ создавать** таблицы в БД других проектов
4. Все ресурсы (сервер, БД, Redis, домен) — свои собственные, отдельные
5. Данные доступа к серверу — только в локальном файле `.server-credentials`
